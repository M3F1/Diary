...
function movieList() {
	...
	$("#iconList > a:nth-child(1)").attr("data-content", "영화1");
...
function inputSchedule(icon) {
	if (icon == "직접 작성") {
		...
		$("#iconList").css("visibility", "hidden");
		...
		if (i == 0) {
			$("#iconList").css("visibility", "visible");
		...
		if (i == 1) {
			...
			$("#tooltiptext").html(q[i]);
			text += "<input type='text' class='clearable' value='" + $("#write").val() + "' /> ";
			// 삭제 아이콘
			// text += "<i class='glyphicon glyphicon-remove'></i>";
			// text += $("#write").val();
		}
		$("#written").html(text);
		$("#written").css("font-size", "150%");
...
/******************************* 날짜 이동 부분 *******************************/
/*function yearSearch() {
	$("#yearSearch").on("mouseenter", function () {
		$("#yearSearch").css("font-size", "90%");

		var html = "";
		var year = $("#yearSearch").html();
		html += "<select id='year'>";
		for (var index = 2000; index < year; index++) {
			html += "<option>" + index + "</option>";

		}
		html += "<option selected='selected'>" + year + "</option>";
		for (var index = parseInt(year) + 1; index < 2051; index++) {
			html += "<option>" + index + "</option>";
		}
		html += "</select>";
		$("#yearSearch").html(html);

		// $("#year").off().on("change", function () {
		// 	alert($("#yearSearch > select > option:selected").html());
		// 	calendarView(new Date(parseInt($("#monthSearch").html()) +
		// 		"-1-" + $("#year > option:selected").html()));
		// 	yearSearch();
		// });
	});

	$("#yearSearch").on("mouseleave", function () {
		$("#yearSearch").css("font-size", "100%");
		var year = $("#yearSearch > select > option:selected").html();
		$("#yearSearch").html(year);
	});
}*/

/*function monthSearch() {
	$("#monthSearch").on("mouseenter", function () {
		$("#monthSearch").css("font-size", "90%");

		var html = "";
		var month = $("#monthSearch").html();
		html += "<select id='month'>";
		for (var index = 1; index < parseInt(month); index++) {
			html += "<option>" + padDigits(index, 2) + "</option>";
		}
		html += "<option selected='selected'>" + month + "</option>";
		for (var index = parseInt(month) + 1; index < 13; index++) {
			html += "<option>" + padDigits(index, 2) + "</option>";
		}
		html += "</select>";
		$("#monthSearch").html(html);

		// $("#month").off().on("change", function () {
		// 	alert($("#month > option:selected").html());
		// 	calendarView(new Date(parseInt($("#month > option:selected").html()) +
		// 		"-1-" + $("#yearSearch").html()));
		// 	monthSearch();
		// });
	});
	...
	$("#monthSearch").on("mouseleave", function () {
		$("#monthSearch").css("font-size", "100%");
		var month = $("#monthSearch > select > option:selected").html();
		$("#monthSearch").html(month);
	});
}*/

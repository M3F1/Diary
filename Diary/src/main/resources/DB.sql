...
DROP SEQUENCE SEQ_ID_USER_TB_idno;
DROP SEQUENCE SEQ_ID_USER_TB_user_no;
DROP SEQUENCE SEQ_ID_USER_TB_user_no_pk;
DROP SEQUENCE SEQ_Member_idno;


/* Create Sequences */

CREATE SEQUENCE SEQ_ID_USER_TB_idno INCREMENT BY 1 START WITH 1;
CREATE SEQUENCE SEQ_ID_USER_TB_user_no INCREMENT BY 1 START WITH 1;
...
(
	...
	sc_ddt date DEFAULT SYSDATE,  				 -- 일정 삭제 날짜  YYMMDD HH24MISS
...
(
	...
	user_birth date NOT NULL,  						-- 회원의 생년월일 정보 (형식은 YYMMDD 6자)
	user_phone number NOT NULL,  					-- 핸드폰 번호 ex)01011111111 (only number)
	...
	user_add1 varchar2(120),  						-- 첫번째 주소 정보(시,군,구)
	user_add2 varchar2(120),  					    -- 2번째 주소정보(시,군,구)
	user_add3 varchar2(120),  					    -- 3번째 주소정보(시,군,구)
	user_ddt date DEFAULT SYSDATE,  			    -- 회원 탈퇴 날짜  YYMMDD HH24MISS
